...
export default function Orders() {
  ...
  const removeSingleItem = (product) => {
    dispatch(removeSingleOrder(product));
  };
          ...
          <div className={cls.bigTitle}>
            ...
            <div className={cls.orders}>
              {items.length > 0 ? (
                items.map((order) => {
                  console.log(order)
                 return (<div className={cls.order} key={order.option.id}>
                    <div className={cls.left}>
                    <div className={cls.product_image__wrapper}>
                      <img src={order.item.imageUrl} />
                    </div>
                      <div className={cls.text}>
                        <p className={cls.top}>
                          <span className={cls.title}>{order.option.name} - </span>
                          <span className={cls.count}>{order.option.quantity}x</span>
                        </p>
                    ...
                    <div className={cls.right}>
                      <p>
                        {formatNumbers(order.option.quantity * order.item.price)}{" "}
                        so&apos;m
                      </p>
                      <div className={cls.buttons}>
                        <RectangeIconButton
                          onClick={() => removeSingleItem(order)}
                        >
                          <Minus size={18} />
                        </RectangeIconButton>
                        <RectangeIconButton
            ...
            <MainButton disabled={disableButtonIsCartEmpty} fullWidth onClick={() => navigate(`/payment`)}>
